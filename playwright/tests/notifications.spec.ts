...
test.describe('Notifications', () => {
  ...
  test('should filter notifications by type', async ({ page }) => {
    ...
    if (await filterButtons.count() > 0) {
      ...
      if (await paymentFilter.isVisible()) {
        ...
        const notificationTypes = await visibleNotifications.locator('[data-testid="notification-type"]').allTextContents();
  ...
  test('should navigate to related transaction from notification', async ({ page }) => {
    ...
    const transactionNotification = page.locator('[data-testid^="notification-item-"][data-type="payment"]').first();
