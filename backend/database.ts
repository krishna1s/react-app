...
export const createUser = (userDetails: Partial<User>): User => {
  ...
  const user: User = {
    ...
    password,
    email: userDetails.email || `${userDetails.username}@example.com`,
    phoneNumber: userDetails.phoneNumber || "555-123-4567",
    balance: Number(userDetails.balance!) || 0,
    avatar: userDetails.avatar || `https://avatars.dicebear.com/api/human/${shortid()}.svg`,
    defaultPrivacyLevel: userDetails.defaultPrivacyLevel || DefaultPrivacyLevel.public,
    createdAt: new Date(),
    modifiedAt: new Date(),
  };
